...
public:
	ShapeAction(std::string name) :
		...
		as_.registerGoalCallback(
			boost::bind(&ShapeAction::goalCB, this));
		as_.registerPreemptCallback(
			boost::bind(&ShapeAction::preemptCB, this));
	...
	void controlCB(const turtlesim::Pose::ConstPtr& msg) {
		...
		if (edge_progress_ < edges_) {
			// scalar values for drive the turtle faster and straigther
			double l_scale = 6.0;
			double a_scale = 6.0;
			...
			dis_error_ = side_len_ - 
				fabs(sqrt((start_x_ - msg->x) * (start_x_ - msg->x) + 
					(start_y_ - msg->y) * (start_y_ - msg->y))
			);

			theta_error_ = angles::normalize_angle_positive(
				M_PI - interior_angle_ - (msg->theta - start_theta_)
			);
			...
			if (dis_error_ > error_tol) {
				...
				command_.linear.x = l_scale * dis_error_;
				command_.angular.z = a_scale * dis_error_;
			}
