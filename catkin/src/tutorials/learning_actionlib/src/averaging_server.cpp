...
public:
	AveragingAction(std::string name) :
		...
		as_.registerGoalCallback(
			boost::bind(&AveragingAction::goalCB, this));
		as_.registerPreemptCallback(
			boost::bind(&AveragingAction::preemptCB, this));

		// subscribe to the data topic of interes
		sub_ = nh_.subscribe("/random_number", 1,
			&AveragingAction::analysisCB, this);
	...
	void analysisCB(const std_msgs::Float32::ConstPtr& msg) {
		...
		feedback_.std_dev = sqrt(fabs((sum_sq_ / data_count_) - 
			pow(feedback_.mean,2)));
