...
namespace simple_layer_namespace 
{
	...
	void GridLayer::updateBounds(double origin_x, double origin_y, double origin_yaw,
								 double* min_x, double* min_y, double* max_x, double* max_y) {
		if (!enabled_)
			return;
		double mark_x = origin_x + cos(origin_yaw);
		double mark_y = origin_y + sin(origin_yaw);
		unsigned int mx;
		unsigned int my;
	...
	void GridLayer::updateCosts(costmap_2d::Costmap2D& master_grid,
